...
#define DEFAULT_RAM_KIB 240
#define MIN_RAM_KIB     176
#define RAM_STEP_KIB    16
#endif
...
#ifdef _RENESAS_SYNERGY_
static byte *AutoAllocMemoryKiB(int *size, int default_kib, int min_kib)
{
    byte *zonemem;
    int attempted_kib;
    ...
    while (zonemem == NULL)
    {
        attempted_kib = default_kib;

        if (attempted_kib < min_kib)
        {
            I_Error("Unable to allocate %i KiB of RAM for zone", attempted_kib + RAM_STEP_KIB);
        }

        *size = attempted_kib * 1024;
        zonemem = malloc((size_t) *size);

        if (zonemem == NULL)
        {
            default_kib -= RAM_STEP_KIB;
        }
    }
...
#ifdef _RENESAS_SYNERGY_
    zone_kib_arg = M_CheckParmWithArgs("-zonekb", 1);

    if (zone_kib_arg > 0)
    {
        default_kib = atoi(myargv[zone_kib_arg + 1]);
        min_kib = default_kib;
    }
    else if (p > 0)
    {
        default_kib = atoi(myargv[p + 1]) * 1024;
        min_kib = default_kib;
    }
    else
    {
        default_kib = DEFAULT_RAM_KIB;
        min_kib = MIN_RAM_KIB;
    }

    zonemem = AutoAllocMemoryKiB(size, default_kib, min_kib);

    printf("zone memory: %p, %u KiB allocated for zone\n",
           zonemem, (unsigned int) (*size / 1024));
#else
...
#ifdef _RENESAS_SYNERGY_
    doom_set_error_msg("I_Quit called");
    while (1)
    {
    }
#endif
