...
static void ExtendLumpInfo(int newnumlumps)
{
    ...
    newsize = (size_t) newnumlumps * sizeof(lumpinfo_t);
    newlumpinfo = Z_Malloc((int) newsize, PU_STATIC, NULL);
    memset(newlumpinfo, 0, newsize);
...
wad_file_t *W_AddFile (char *filename)
{
    ...
    else 
    {
        ...
        {
            mapped_fileinfo = wad_file->mapped + header.infotableofs;
        }
        else
        {
    ...
    {
        filelump_t lump_entry;

        if (mapped_fileinfo != NULL)
        {
            memcpy(&lump_entry,
                   mapped_fileinfo + ((i - (unsigned int) startlump) * sizeof(filelump_t)),
                   sizeof(lump_entry));
            filerover = &lump_entry;
        }
        else
        {
            filerover = &fileinfo[i - (unsigned int) startlump];
        }
